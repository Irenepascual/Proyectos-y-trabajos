...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(X_RANGE):
        for j in range(Y_RANGE):
            lista.append((i,j)) #lista donde guardamos todas las posibles posiciones en el tablero

    variableDomainsDict = {
        PAC: lista,  # todas las posibles posiciones del pacman
        GHOST0: lista,  # todas las posibles posiciones de Ghost0
        GHOST1: lista,  # todas las posibles posiciones de Ghost1
        OBS0: list(i for i in range(X_RANGE + Y_RANGE + MAX_NOISE - 1)),  # ruido alrededor de la distancia real de OBS0
        OBS1: list(i for i in range(X_RANGE + Y_RANGE + MAX_NOISE - 1)),  # ruido alrededor de la distancia real de OBS1
    }
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        #extraemos factores con evidencia
        factores_con_evidencia = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        #seleccionamos solo los factores que involucran la variable que queremos eliminar
        for variable_a_eliminar in eliminationOrder:
            factores_relacionados  = []
            for factor in factores_con_evidencia:
                if variable_a_eliminar in factor.variablesSet():  
                    factores_relacionados .append(factor)

            #combinamos las probabilidades de múltiples tablas (de todos los factores que 
            #contienen la variable a eliminar) considerando todas las variables
            #involucradas --> obtenemos la unión, que es la probabilidad combinada
            if factores_relacionados :
                #factores_con_evidencia: actualiza su lista de factores sustituyendo por el nuevo factor_unido
                #factor_unido: resultado de unir todas las tablas de probs que involucraban variable_a_eliminar
                factores_con_evidencia, factor_unido = joinFactorsByVariable(factores_con_evidencia, variable_a_eliminar)              

                #################  MARGINALIZAR:  ###################
                #verifica que hay más de una variable sin condición antes de eliminar una
                #variable intermedia--> asegurando que el factor resultante siga siendo válido
                #y útil para los cálculos. 
                #Se debe verificar antes que hay más de una variable sin condición porque sino
                #no tendría sentido calcularlo ya que el factor sería como un número constante. 
                if len(factor_unido.unconditionedVariables()) > 1:
                    factores_con_evidencia.append(eliminate(factor_unido, variable_a_eliminar))

        #unión final de factores para representar la probabiliadd conjunta
        if len(factores_con_evidencia) > 1:
            factor_final = joinFactors(factores_con_evidencia)
        else:
            factor_final = factores_con_evidencia[0]
        
        #################  NORMALIZAR: ##################
        #ajusta las probabilidades conjuntas para garantizar que sean válidas y sumen 1
        return normalize(factor_final)
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #############  CÓDIGO DE HACER LA NORMALIZACIÓN : ###############

        #Calcular la suma de todos los valores en la distribución
        suma_tot = sum(self.values())
        
        #Si la suma es 0, no se hace nada
        if suma_tot == 0:
            return
        
        #Normalizar cada valor dividiendo por la suma total
        #Así logramos que todo sume 1
        for clave in self:
            self[clave] = self[clave] / suma_tot
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #vamos a hacer una muestra aleatoria ponderada basada en las probabilidades
        #asociadas a las claves de la distribución

        #Calcula el total de las probabilidades
        suma_tot = sum(self.values())
        
        #Genera un número aleatorio entre 0 y el total de la distribución, que ayudará
        #a decidir qué clave (variable) elegir
        aleatorio = random.random() * suma_tot
        
        suma_acumulada = 0        
        #Acumulamos las probabilidades de cada clave mientras iteramos sobre los 
        #elementos de la distribución. Cada vez que la suma acumulada supere el valor
        #aleatorio generado, esa clave se devuelve como resultado. 
        #Esto es porque las claves con valores más altos tienen más probabilidad de ser
        #seleccionadas.
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        #Calcular la probabilidad condicionada de la distancia ruidosa dada la posición del pacman y del fantasma
        
        #El fantasma está en la carcel
        if ghostPosition == jailPosition:
            #si no se tiene información sobre la distancia, seguro está en la carcel
            if noisyDistance is None:
                return 1.0
            #si tenemos información sobre la distancia, seguro NO está en la cárcel
            else:
                return 0.0

        #Si no hay ruido, la probabilidad de la observación es 0 ya que no podemos tener 
        #una distancia ruidosa observada si no sabemos la posición
        if noisyDistance is None:
            return 0.0

        #calculamos la distancia real entre el pacman y el fantasma
        distancia = manhattanDistance(pacmanPosition, ghostPosition)

        #obtenemos la probabilidad de la observación ruidosa sabiendo la distancia
        return busters.getObservationProbability(noisyDistance, distancia)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        posicion_jail = self.getJailPosition() 

        #para cada posible posición del fantasma, se calcula la probabilidad de observación
        #dada esa posición--> esto se usa para actualizar la creencia sobre la posición
        #del fantasma multiplicando por la probabilidad de observación.
        for posicion_ghost in self.allPositions:
            #calcula la probabilidad de que pacman haya observado una distancia ruidosa dada una posición 
            #específica del fantasma, del pacman y de esa posicion del fantasma.
            probabilidad = self.getObservationProb(observation, posicion_pacman, posicion_ghost, posicion_jail)

            #usa la probabilidad anterior multiplicado por la nueva probabilidad
            self.beliefs[posicion_ghost] = self.beliefs[posicion_ghost] * probabilidad  
