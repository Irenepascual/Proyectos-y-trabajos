...
func TestAcuerdosConFallos(t *testing.T) { // (m *testing.M) {
	...
	t.Run("T8:RecuperacionEstadoReplica ",
		func(t *testing.T) { cfg.RecuperacionEstadoReplica(t) })
...
// Se consigue acuerdo a pesar de desconexiones de seguidor -- 3 NODOS RAFT
func (cfg *configDespliegue) RecuperacionEstadoReplica(t *testing.T) {
	//t.Skip("SKIPPED RecuperacionEstadoReplica")
...
func (cfg *configDespliegue) reconectarNodo() {
	for i, _ := range cfg.nodosRaft {
		if !cfg.conectados[i] {
			cfg.t.Log("Vamos a reconectar el nodo", i)
