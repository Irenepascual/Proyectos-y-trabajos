...
func getHostKey(host string) ssh.PublicKey {
	...
	file, err := os.Open(filepath.Join(os.Getenv("HOME"), ".ssh",
						"known_hosts"))
	if err != nil {
		log.Fatal(err)
	}
	...
	for scanner.Scan() {
		...
		if strings.Contains(fields[0], host) {
			var err error
			hostKey, _, _, _, err =
				ssh.ParseAuthorizedKey(scanner.Bytes())
			if err != nil {
...
func executeCmd(hostname string, cmd string, session *ssh.Session) string {
	...
	fullCmd := "export PATH=$PATH:/usr/local/go/bin; " + cmd
	session.Run(fullCmd)
...
func getSigner(pkeyFile string) (ssh.Signer, error) {
	...
	pkey, err := os.ReadFile(filepath.Join(os.Getenv("HOME"),
						".ssh", pkeyFile))
...
func execOneHost(hostname string, results chan<- string, cmd string) {
	//fmt.Println("ssh....go: dentro de execOneHost(): host:", hostname)
	...
	for _, pkeyOneFile := range pkeyFiles {
		...
		if errSigner == nil {
			...
			//fmt.Println("Se ha creado la sesión ssh de la máquina", hostname)
	...
	log.Fatalf("NO funciona conexión ssh con clave pública a %s",
							hostname)
}
