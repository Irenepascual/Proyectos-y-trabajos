...
func (nr *NodoRaft) someterOperacion(operacion TipoOperacion) (int, int,
	...
	nr.IndiceReg++
	nr.Reg = append(nr.Reg, TipoReg{mandato, operacion})
...
func (nr *NodoRaft) PedirVoto(peticion *ArgsPeticionVoto, reply *RespuestaPeticionVoto) error {
	...
	// 3. Si nuestro mandato == su mandato y ya habiamos votado a alguien
	if nr.MandatoActual == peticion.Mandato && nr.VotadoA != -1 {
	...
	// 4. Si nuestro mandato == su mandato y no hemos votado a nadie aun
	if nr.MandatoActual == peticion.Mandato && nr.VotadoA == -1 {
		// ELEGIR VOTO SEGUN LOS REGISTROS
		nr.Logger.Println("error: elecciones cruzadas")
		reply.VotoConcedido = false
		reply.Mandato = nr.MandatoActual
...
func (nr *NodoRaft) AppendEntries(args *ArgAppendEntries,
	...
	if args.Entradas == (TipoReg{}) {
		...
		if args.Mandato < nr.MandatoActual {
			nr.Logger.Println("AppendEntries: nosotros vamos más adelantados",
				nr.MandatoActual, "que el", args.Mandato)
			results.Mandato = nr.MandatoActual
			results.Exito = false

			// 1.2. Voy igual que el líder
		} else if args.Mandato == nr.MandatoActual {
			nr.Logger.Println("AppendEntries: recibido un latido en mandato",
			...
			// 1.3. Voy más retrasado que el líder
		} else {
			nr.Logger.Println("AppendEntries: nosotros vamos retrasados", nr.MandatoActual, "que el", args.Mandato, "=> nos actualizamos")
			nr.IdLider = args.IdLider
		...
		nr.Logger.Printf("recibida la entrada:",
			args.Entradas.Mandato,
			args.Entradas.Operacion.Operacion,
			args.Entradas.Operacion.Clave,
			args.Entradas.Operacion.Valor)
		// AÑADIR COMO TRATAR LA APLICACIÓN DE LA ENTRADA A LA MAQUINA DE ESTADOS
		results.Mandato = nr.MandatoActual // CAMBIAR
		results.Exito = true               // CAMBIAR
	}
	return nil
}
...
func (nr *NodoRaft) iniciarEleccion() {
	...
	for i := 0; i < len(nr.Nodos); i++ {
		if i != nr.Yo && nr.EstadoVital != Caido {
			go nr.enviarPeticionVoto(i, &ArgsPeticionVoto{nr.MandatoActual, nr.Yo}, &reply)
		}
	}
}
...
func (nr *NodoRaft) enviar1Latido(nodo int, args *ArgAppendEntries,
	...
	if err == nil {
		...
		if results.Mandato > nr.MandatoActual {
			nr.Logger.Println("raft.go: nos han respondido al latido y estamos desactualizados (nuestro mandato:",
				nr.MandatoActual, ", su mandato",
				results.Mandato, ")")
