...
* FICHERO: worker3.go
* DESCRIPCIÓN: fichero worker de la arquitectura máster-worker
 */
package main
...
func main() {
	args := os.Args
	if len(args) != 2 {
		log.Println("W: Error: endpoint missing: go run worker3.go ip:port")
		os.Exit(1)
	}
	endpoint := args[1]
	...
	log.Println("W: ***** Listening for new connection in endpoint ", endpoint)
