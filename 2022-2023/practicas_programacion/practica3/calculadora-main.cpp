...
void presentarMenu() {
    ...
    cout << "9 - Comprobar si el número es capicúa" << endl;
    cout << endl;
}
...
/*
 * Pre:  ---
 * Post: Ha ejecutado la 6ª orden, donde se escribe el factorial del número dado.
 */
void ejecutarFactorial(int numero) {
    if(numero>=0){
        cout << "El número factorial de " << numero << " es " << factorial(numero) << endl;
    }
    else{
...
/*
 * Pre:  ---
 * Post: Ha ejecutado la 7ª orden, y pide otro número (que no pueden ser 0 a la vez), 
 *       para hacer su máximo común divisor. 
 */
void ejecutarmcd(int numero1, int numero2) {
    ...
    if(numero1==0 && numero2==0){
        cout << "No pueden ser a la vez 0." << endl;
    }
    else{
        cout << "El máximo común divisor (MCD) de " << numero1 << " y" << numero2 << " es " << mcd(numero1, numero2) << endl;
    }
}

/*
 * Pre:  ---
 * Post: Ha ejecutado la 8ª orden, y pide otro número (que no pueden ser 0 a la vez),
 *       para hacer su mínimo común múltiplo.
 */
void ejecutarmcm(int numero1, int numero2) {
    ...
    if(numero1==0 && numero2==0){
        ...
        cout << "El mínimo común múltiplo (MCM) de " << numero1 << " y " << numero2 << " es " << mcm(numero1, numero2) << endl;
    }
}

/*
 * Pre:  ---
 * Post: Ha ejecutado la 9ª orden, informando sobre si «numero» es un número capicúa
 *       o no lo es.
 */
...
/*
 * Pre:  ---
 * Post: Dependiendo del número que se introduce, se realiza una operación u otra.
 */
void ejecutarOrden(unsigned operacion) {
    if (operacion >= 1 && operacion <= NUM_OPERACIONES) {
        ...
        if (operacion == 1) {
            ...
            ejecutarmcd(numero, numero);
        }
        else if (operacion == 8) {
            ejecutarmcm(numero, numero);
        }
        else if (operacion == 9) {
